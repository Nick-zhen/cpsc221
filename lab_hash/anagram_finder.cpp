...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
     ...
     */
    Dict<char, int> dictWord(200);
    Dict<char, int> dictTest(200);
    
    for (size_t i = 0; i < word.length(); i++) {
        // if (!keyExists(word[i])) {
        //     dictWord.insert(word[i], 1);
        // } else {
        //     dictWord[word[i]]++;
        // }
        dictWord[word[i]]++;
    ...
    for (it = dictWord.begin(); it != dictWord.end(); it++) {
        if (dictWord[it->first] != dictTest[it->first]) {
            return false;
        }
    }
    return true;
